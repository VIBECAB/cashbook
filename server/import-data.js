...
// Get business IDs
const inparlor = db.prepare("SELECT id FROM businesses WHERE name = 'InParlor'").get();
...
const importAll = db.transaction(() => {
  // Clear existing ledger and transaction data
  db.exec('DELETE FROM partner_ledger');
  db.exec('DELETE FROM employee_expenses');
  db.exec('DELETE FROM employee_budgets');
  db.exec('DELETE FROM transactions');

  let count = 0;

  // ============================================================
  // PDF 1: Sajid Credit Card Visa (Sajid owes Akram)
  // Cash out = credit (Sajid owes Akram)
  // Cash in = debit (Sajid paid back / reduced debt)
  // ============================================================
  const sajidEntries = [
    // Page 1
  ...
  for (const e of sajidEntries) {
    insertLedger.run(akram.id, sajid.id, e.type, e.amount, e.desc, e.date);
    count++;
  }
  ...
  // ============================================================
  // PDF 2: Hammad Credit Cards (Hammad owes Akram)
  // Cash out = credit (Hammad owes Akram)
  // Cash in = debit (Hammad paid back / reduced debt)
  // ============================================================
  const hammadEntries = [
    // Page 1
  ...
  for (const e of hammadEntries) {
    insertLedger.run(akram.id, hammad.id, e.type, e.amount, e.desc, e.date);
    count++;
  }

  // Verify Hammad totals
  ...
  // ============================================================
  // PDF 3: Office Register - Expenses by Akram (March 2026, InParlor)
  // ============================================================
  console.log('\nInserting 3 InParlor expense entries by Akram...');
  insertTransaction.run(inparlor.id, akram.id, 'expense', 'personal', 5000, 'Junaid kharcha', 'Miscellaneous', '2026-03-02');
  insertTransaction.run(inparlor.id, akram.id, 'expense', 'personal', 2000, 'Junaid advance', 'Miscellaneous', '2026-03-03');
  insertTransaction.run(inparlor.id, akram.id, 'expense', 'personal', 6272, 'Net bill', 'Utilities', '2026-03-05');
  count += 3;
...
const sajidNet = db.prepare(`
  ...
  FROM partner_ledger WHERE created_by = ? AND partner_id = ?
`).get(akram.id, sajid.id);
console.log(`\nSajid owes Akram: Rs ${sajidNet.credits - sajidNet.debits} (expected: 1280134)`);

const hammadNet = db.prepare(`
  SELECT
    COALESCE(SUM(CASE WHEN type='credit' THEN amount ELSE 0 END), 0) as credits,
    COALESCE(SUM(CASE WHEN type='debit' THEN amount ELSE 0 END), 0) as debits
  FROM partner_ledger WHERE created_by = ? AND partner_id = ?
`).get(akram.id, hammad.id);
console.log(`Hammad owes Akram: Rs ${hammadNet.credits - hammadNet.debits} (expected: 1788727)`);
